...
管理端侧栏和顶栏都有「基地大屏」，打开 `/big-screen`。默认一屏放指标、棚区平面、抓拍墙、告警与环境；另可切到抓拍墙或带时间轴的多区。时间轴进入生长趋势，或按上海日筛选识别记录。这一页使用夜色投屏皮肤（`data-skin=tb-night`），不使用雪亮配色，也不改管理端浅色运营台。数据来自现有总览、棚区、告警、设备、识别列表和生长趋势接口。
