...
| 2026-09-23 | #43 认领与误报关闭：POST /alerts/:id/claim 写入 claimedBy；POST /alerts/:id/false-positive 以 closeReason=false_positive 关闭，普通关闭为 resolved。备注进审计。查看员与棚外 403。`/alerts` 沿用浅色 panel / btn-ghost | `make test` 退出码 0（API jest 63，web vitest 63）；`make gates` 退出码 0 | 本机未起登录后的 /alerts。PR 未合，不关 Issue #43 |
