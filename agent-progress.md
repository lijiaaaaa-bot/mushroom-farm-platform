...
## 当前
- 本会话目标：识别与环境补传窗口 `LIMITS.recognizedAtPastDays` 改为 90 天，历史报文走 HTTP/MQTT 校验
- 进行中文件：`packages/contracts/src/index.ts`、`packages/contracts/src/contracts.spec.ts`、`STATUS.md`
...
| 2026-09-23 | 识别与环境补传窗口改为 90 天（`LIMITS.recognizedAtPastDays` = `TIMESERIES_RETENTION_DAYS`）。满 90 天整点仍收，超出 1ms 拒收并返回补传窗口错误。8 天前的报文可通过解析 | 待 `make test` 与 `make gates` | 不改幂等 TTL，不改大屏/报表的 7 日查询窗 |
