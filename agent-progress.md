...
| 2026-09-24 | 死代码审计。Issue #73：(A) 删无引用 Phase2View、phase2/trends 与 phase2/yield 重定向、placeholder phase2 API、空 app.controller.spec 与空 e2e；(B) daily_aggregates 仍双写，读已走桶；(C) object-lifecycle 仍打印未下发，phase2/wecom 与 phase2/big-screen 重定向保留 | 待 `make test` 与 `make gates` | 不关 #73。不删桶读写，不删对象生命周期脚本 |
