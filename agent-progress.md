...
| 2026-09-23 | #23 阈值规则页接到已有 POST/PATCH `/alert-rules`。不改 AlertsController / AlertEngine。写权限仅超管与生产管理员 | `apps/api` alert-rules.spec；`apps/web` RulesView.spec；`make test`；`make gates` | PR 未合，不关 Issue #23 |
